/* uLisp AVR Version 2.8 beta - www.ulisp.com
...
ANALOGREAD, ANALOGWRITE, DELAY, MILLIS, SLEEP, NOTE, ATTACHINTERRUPT, EDIT, PPRINT, PPRINTALL, REQUIRE,
...
// Flags
enum flag { RETURNFLAG, ESCAPE, EXITEDITOR, LIBRARYLOADED, INTERRUPT, BUSY };
volatile char Flags;
...
object *fn_attachinterrupt (object *args, object *env) {
  ...
  if (n<0 || n>=NINTERRUPTS) error(PSTR("'attach-interrupt' invalid interrupt"));
  args = cdr(args);
  ...
  if (mode<0 || mode>3) error(PSTR("'attach-interrupt' invalid mode"));
  EIMSK |= 1<<n;
  n = n<<1;
...
const char string155[] PROGMEM = "attach-interrupt";
const char string156[] PROGMEM = "edit";
const char string157[] PROGMEM = "pprint";
const char string158[] PROGMEM = "pprintall";
...
const tbl_entry_t lookup_table[] PROGMEM = {
  ...
  { string155, fn_attachinterrupt, 1, 3 },
  { string156, fn_edit, 1, 1 },
  { string157, fn_pprint, 1, 2 },
  { string158, fn_pprintall, 0, 0 },
  { string159, fn_require, 1, 1 },
  { string160, fn_listlibrary, 0, 0 },
};
...
int gserial () {
  ...
  while (!Serial.available()) handleInterrupts();
...
void setup () {
  ...
  initenv();
  initsleep();
  pfstring(PSTR("uLisp 2.8 beta "), pserial); pln(pserial);
}
